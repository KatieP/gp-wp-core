	...
	function links(){
		...
		if (($this->pages) > 1){
			...
			if ($this->page != 1) {
				...
				$plinks[] = '<a href="?page=1'.$queryURL.$this->searchFor.'" id="pageLink" class="pageLink">'.$this->first.'</a>';
				}
				$plinks[] = '&nbsp;<a href="?page='.($this->page - 1).$queryURL.$this->searchFor.'" id="pageLink" class="pageLink">'.$this->prev.'</a>&nbsp;';
			}
			...
			for ($j = 1; $j < ($this->pages + 1); $j++) {
				if ($this->page == $j) {
					...
					$links[] = ' <a href="?page='.$j.$queryURL.$this->searchFor.'" id="pageLink" class="pageLink">'.$j.'</a> '; // add the link to the array
				}
			}
			...
			if ($this->page < $this->pages) {
				$slinks[] = '&nbsp;<a href="?page='.($this->page + 1).$queryURL.$this->searchFor.'" id="pageLink" class="pageLink">'.$this->next.'</a>&nbsp;';
				if ($this->showFirstAndLast) {
					$slinks[] = '<a href="?page='.($this->pages).$queryURL.$this->searchFor.'" id="pageLink" class="pageLink">'.$this->last.'</a>';
				}
			}
