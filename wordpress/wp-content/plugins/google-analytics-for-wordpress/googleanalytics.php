...
<?php
		...
		function ga_get_tracking_prefix() {
			...
			return (empty($options['trackprefix'])) ? '/yoast-ga/' . $post_id . '/' . $post_author_id . '/' : $options['trackprefix'] . $post_id . '/' . $post_author_id . '/';
		}
		...
		function ga_get_tracking_link($prefix, $target, $jsprefix = 'javascript:') {
				...
				( $prefix == 'download' && $options['downloadspageview'] ) ) 
			{
				...
				$pushstr = "['_trackPageview','".$prefix."/']"; // JB Removed target url so that we can retreive results from GA
			} else {
