...
if ( !class_exists( 'GA_Filter' ) ) {
	class GA_Filter {
		...
		function ga_get_tracking_prefix() {
		    global $post; // GP EDIT
		    $post_author = get_userdata($post->post_author); // GP EDIT
		    $post_id = $post->ID; // GP EDIT
		    $post_author_id = $post_author->ID; // GP EDIT
		    
			$options = get_option( 'Yoast_Google_Analytics' );
			#return ( empty( $options['trackprefix'] ) ) ? '/yoast-ga/' : $options['trackprefix'];
			return (empty($options['trackprefix'])) ? '/yoast-ga/' . $post_id . '/' . $post_author_id . '/' : $options['trackprefix'] . $post_id . '/' . $post_author_id . '/'; // GP EDIT
		}
		...
		function ga_get_tracking_link( $prefix, $target, $jsprefix = 'javascript:' ) {
			...
			if (
				...
				$pushstr = "['_trackPageview','".$prefix."/']"; // GP EDIT - JB Removed target url so that we can retreive results from GA
			} else {
