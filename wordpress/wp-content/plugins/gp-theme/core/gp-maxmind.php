...
function gp_core_create_maxmind_tables() {
	...
	$sql[] = "DROP TABLE IF EXISTS " . $wpdb->base_prefix . "maxmind_geolitecityblocks;";
	
	$sql[] = "CREATE TABLE " . $wpdb->base_prefix . "maxmind_geolitecityblocks (
	...
	$sql[] = "CREATE TABLE " . $wpdb->base_prefix . "maxmind_geolitecitylocation (
		...
		country VARCHAR(2) DEFAULT NULL,
  		...
  		dmaCode VARCHAR(3) DEFAULT NULL,
...
function gp_core_import_maxmind_citiesdata() {
	...
	$query = "DELIMITER $$
		...
		BEGIN
		  ...
		  SELECT locID INTO locID_var
		    FROM wp_maxmind_geolitecityblocks
		    INNER JOIN
	...
	*/
	...
	$query = "LOAD data LOCAL INFILE '" . GP_PLUGIN_DIR . "/import/GeoLiteCity-Blocks.csv' 
		INTO TABLE " . $wpdb->base_prefix . "maxmind_geolitecityblocks 
